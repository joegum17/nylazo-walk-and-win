...
export function BossArena({ boss, gender, steps, onWin, onFlee }: Props) {
  ...
  // damage = base 30 + steps/10, min 25
  ...
  const effectLayer = useMemo(() => {
    if (def.effect === "rain") {
      return Array.from({ length: 40 }).map((_, i) => (
        <div
          key={i}
          className="absolute w-[2px] rounded bg-sky-200/80"
          style={{
            left: `${(i * 2.7) % 100}%`,
            height: 18,
            top: "-20px",
      ...
      return Array.from({ length: 50 }).map((_, i) => (
        <div
          ...
          style={{
            ...
            animation: `snow-drift ${4 + Math.random() * 3}s linear ${Math.random() * 3}s infinite`,
          }}
        >
          ❄
        </div>
    ...
    if (def.effect === "wind") {
      return Array.from({ length: 12 }).map((_, i) => (
        <div
          key={i}
          className="absolute h-[2px] w-12 rounded bg-white/70"
          style={{
            left: `${Math.random() * 80}%`,
    ...
    if (def.effect === "germ") {
      return Array.from({ length: 18 }).map((_, i) => (
        <div
          key={i}
          className="absolute h-3 w-3 rounded-full bg-lime-400/70"
          style={{
    ...
    <div className="relative flex min-h-[80vh] flex-col p-4">
      ...
      <div className="parchment-panel mx-auto w-full max-w-xl rounded-xl p-3">
        <div className="mb-1 flex items-center justify-between text-sm">
          ...
          <span className="text-xs text-muted-foreground">HP {hp}/{def.hp}</span>
        </div>
      ...
      {/* Arena */}
      <div className="relative mx-auto mt-6 flex w-full max-w-2xl flex-1 items-end justify-between overflow-hidden rounded-2xl border-2 border-[var(--wood-deep)] p-4" style={{ minHeight: 360, background: "linear-gradient(180deg, oklch(0.84 0.07 200) 0%, oklch(0.86 0.08 130) 70%, oklch(0.55 0.10 100) 100%)" }}>
        {effectLayer}
          ...
          {!won ? (
            <>
              ...
              {floats.map((f) => (
                <div
                  key={f.id}
                  className="pointer-events-none absolute text-2xl font-black"
                  style={{
                    left: `${f.x}%`,
                    top: 30,
                    color: "#fff",
                    textShadow: "0 0 6px #b91c1c, 0 2px 0 #000",
      ...
      {won && (
        <div className="fixed inset-0 z-30 grid place-items-center bg-black/50">
          <div className="parchment-panel animate-burst rounded-2xl p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">ได้รับ "โล่รางวัล" +1</p>
          </div>
        </div>
      )}
    </div>
